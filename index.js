...
var accepted_cp = ["75013", "75002", "93000", "75001"]
  // prix
...
var latest = items.map(function (a){
  ...
  //poids des items
var weight = items.map(function (a){
   return a.weight
     })  
  // nombre de colis
...
var cp_ok = cp.filter(cp=>{
    return accepted_cp.includes(cp)
      })
  // Tarification des CP
var nbAdresseDansParis = adresse.map(function (a) {
    return `${a.postalCode}`
      }).filter(function (cp) {
